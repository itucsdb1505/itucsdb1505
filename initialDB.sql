...
CREATE TABLE POOL(NAME VARCHAR(40) NOT NULL, COUNTRY_ID INT, CAPACITY INT NOT NULL, BUILT INT NOT NULL,ID SERIAL PRIMARY KEY);
...
CREATE TABLE STATS(PLAYER_ID INT,LEAGUE_ID INT, GOAL INT DEFAULT 0, ASSIST INT DEFAULT 0,SAVE INT DEFAULT 0,ID SERIAL PRIMARY KEY);
...
CREATE TABLE MESSAGES(ID SERIAL PRIMARY KEY, TITLE VARCHAR(100) NOT NULL, MAIL VARCHAR(100), MESSAGE TEXT NOT NULL);
...
INSERT INTO NEWS(TITLE, CONTENT, USER_ID) values('New 4', 'New 4 Content', 1);
...
INSERT INTO MESSAGES(TITLE, MAIL, MESSAGE) values('Tebrik','mail@mail.com','Site için sizi tebrik eder başarılarınızın devamını dilerim... :)');
...
ALTER TABLE NEWS ADD CONSTRAINT FK_NEWS_USERS FOREIGN KEY (USER_ID) REFERENCES USERS (ID) ON DELETE SET NULL ON UPDATE CASCADE;
...
ALTER TABLE STATS ADD CONSTRAINT FK_STATS_PLAYERS FOREIGN KEY (PLAYER_ID) REFERENCES PLAYERS (ID) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE STATS ADD CONSTRAINT FK_STATS_LEAGUES FOREIGN KEY (LEAGUE_ID) REFERENCES LEAGUES (ID) ON DELETE CASCADE ON UPDATE CASCADE;
